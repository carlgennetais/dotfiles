...
if [ ${#WINDOW_IDS[@]} -eq 0 ]; then
    if [ "$NO_LAUNCH" = true ]; then
        ...
        # Wait for the window to appear, then focus it
        for _ in $(seq 1 50); do
            sleep 0.1
            if [ -n "$TITLE_SEARCH" ]; then
                NEW_ID=$(niri msg --json windows | jq -r --arg query "$TITLE_SEARCH" \
                    '[.[] | select(.title and (.title | test($query; "i")))] | .[0].id // empty')
            else
                NEW_ID=$(niri msg --json windows | jq -r --arg query "$SEARCH_CLASS" \
                    '[.[] | select(.app_id and (.app_id | test($query; "i")))] | .[0].id // empty')
            fi
            if [ -n "$NEW_ID" ]; then
